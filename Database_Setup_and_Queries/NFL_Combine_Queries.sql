...
-- Find the average stats of each conference (power five only)
SELECT S.conference
	  ,ROUND(AVG(N.height),2) AS avg_height
	  ,ROUND(AVG(N.weight),2) AS avg_weight
	  ,ROUND(AVG(N.sprint_40yd),2) AS avg_sprint_40yd
...
JOIN public.school_by_conference S
ON S.school = N.school
WHERE S.conference IN ('ACC','Big Ten','Big 12','Pac-12','SEC')
GROUP BY S.conference
ORDER BY S.conference;
...
-- Find the number of drafted and undrafted players from each conference (power five only)
SELECT S.conference
	  ,N.drafted
	  ,COUNT(N.player) AS player_count
FROM public.nfl N
